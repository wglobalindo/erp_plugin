...
// For supplier Ref input by Jay 211012
$mg_ref_supplier = GETPOST('mg_ref_supplier', 'alpha');
...
$hookmanager->initHooks(array('supplierorderlist_new'));
	...
	if ($massaction == 'confirm_createsupplierbills')
	{
		...
		foreach ($orders as $id_order) {
			...
			if (!empty($createbills_onebythird) && !empty($TFactThird[$cmd->socid])) $objecttmp = $TFactThird[$cmd->socid]; // If option "one bill per third" is set, we use already created order.
				...
				if (empty($mg_ref_supplier)) {
					$mg_ref_supplier=$cmd->ref_supplier;
				}

				$objecttmp->ref_supplier =$mg_ref_supplier;
				...
				if ($res <= 0) {
					$error++;
					if (empty($mg_ref_supplier)) {
						setEventMessages('Supplier Ref Empty', null, 'errors');
						break;
					}else{
						setEventMessages($objecttmp->error, null, 'errors');
						break;
					}
				}
...
if ($resql)
{
	...
	if ($massaction == 'createbills')
	{
		...
		print '<tr>';
		print '<td class="titlefield">';
		print $langs->trans('RefOrderSupplierShort');
		print '</td>';
		print '<td>';
		//print $form->selectDate('', '', '', '', '', '', 1, 1);
		print '<input class="flat  valignmiddle" type="text" maxlength="20" name="mg_ref_supplier" value="'.$mg_ref_supplier.'">';
		print '</td>';
		print '</tr>';
