	...
	if ($massaction == 'confirm_createbills') {
		...
		foreach ($receptions as $id_reception)
		{
			...
			if (!empty($createbills_onebythird) && !empty($TFactThird[$rcp->socid])) {
				...
				{
					foreach ($rcp->linkedObjectsIds['repception'] as $key => $value)
					{
					...
					if ($rcp->ref_supplier =="") {
						$object->ref_supplier = $rcp->ref;
					}
				}
				//$object->multicurrency_code =$rcp->multicurrency_code;
			} else {
					...
					for ($i = 0; $i < $num; $i++)
					{
						...
						if ($lines[$i]->subprice < 0)
						{
							...
							if (method_exists($lines[$i], 'fetch_optionals')) 							// For avoid conflicts if
							{
								...
								$srcLine = new CommandeFournisseurDispatch($db);
								...
								$array_options = array_merge($lines[$i]->array_options, $srcLine->array_options);
...
$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."delivery as l ON l.rowid = el.fk_target";
