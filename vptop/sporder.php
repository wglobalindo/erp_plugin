...
if (empty($reshook))
{
	...
	if ($action == 'add' && $usercancreate)
	{
		...
		if (!$error)
		{
			...
			// check create with socid
			if (GETPOST('multicurrency_code', 'alpha') == "none"){
				$soc = new societe($db);
				$soc->fetch($socid);
				$object->multicurrency_code = $soc->multicurrency_code;
			} else{
		   	...
		   	if (!$error)
		   	{
				...
				if (!empty($origin) && !empty($originid))
				{
					...
					if ($id > 0)
					{
						...
						if ($result > 0)
						{
							//ExtraField Update when supplier order
							$srcobject->array_options=array('options_rq'=>'poprocess');
							$srcobject->updateExtraField('rq');
							...
							for ($i = 0; $i < $num; $i++)
							{
								...
								if (method_exists($lines[$i], 'fetch_optionals')) 							// For avoid conflicts if
								{
									...
									$array_option =array('options_cs'=>$srcobject->socid, 'options_cp'=>$srcobject->id, 'options_unit'=>$lines[$i]->array_options['options_unit']);
                                	//$lines[$i]->array_option['options_cs']= $srcobject->socid;
								}
								...
								$result = $object->addline(
                                    ...
                                    $lines[$i]->array_options['options_spd'],
									'HT',
									0,
									...
									$array_option,
									$lines[$i]->fk_unit,
									$lines[$i]->array_options['options_spp'],
									$array_option,
			...
			if ($error)
			{
				...
				header("Location: /fourn/commande/card.php?id=".$id);
				exit;
			}
		}
	}
...
if ($action == 'create')
{
	...
	if (!empty($origin) && !empty($originid))
	{
		...
		//custom
		dol_include_once('/custom/vptop/class/Utill.php');

		//$objectsrc = new $classname($db);
		$objectsrc = new Commande2($db);
		...
		$object->array_options['options_cspo'] = $objectsrc->id;
		$object->array_options['options_cs'] = $objectsrc->socid;
		...
		{
			if (!empty($objectsrc->multicurrency_code)) $currency_code = $societe->multicurrency_code;
	...
	if (!empty($conf->multicurrency->enabled))
	{
		...
		// when create without socid
		print '<input type="hidden" name="multicurrency_code" value="none">';
	}
