...
if (($action == 'create' || $action == 'add') && !$error)
{
	...
	$result = restrictedArea($user, 'commande', $id,'','','fk_soc',$fieldid);
...
if (($action != 'create' && $action != 'add') || ($action == 'create' && $error))
{
	...
	if ($resql)
	{
		...
		print '<form method="get" action="merge.php">';
		...
		print '<form name="orders2invoice" action="merge.php" method="GET">';
		...
		*/
		...
		print '<input type="submit" class="butAction" value="'.$langs->trans("Costomer po merge").'">';
		print '</div>';
		print '</div>';
		print '</form>';
		$db->free($resql);
