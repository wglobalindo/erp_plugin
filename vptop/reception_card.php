...
$hookmanager->initHooks(array('receptioncard_new', 'globalcard'));
...
if (empty($reshook))
{
	...
	if ($action == 'add' && $user->rights->reception->creer)
	{
		...
		if ($totalqty > 0)  // There is at least one thing to ship
		{
			...
			for ($i = 0; $i <= $num; $i++)
			{
				...
				if (GETPOST($qty, 'int') > 0 || (GETPOST($qty, 'int') == 0 && $conf->global->RECEPTION_GETS_ALL_ORDER_PRODUCTS))
				{
					...
					$array_options[$i]['options_vppo']=$objectsrc->id;
		...
		{
			$db->commit();
			header("Location: reception_card.php?id=".$object->id);
			exit;
		} else {
			$db->rollback();
...
if ($action == 'create')
{
	...
	if ($origin)
	{
		...
		if ($objectsrc->fetch($origin_id)) {	// This include the fetch_lines
			...
			// Reception lines
			$numAsked = count($objectsrc->lines);
			$dispatchLines = array();
			foreach ($_POST as $key => $value) {
			...
			while ($indiceAsked < $numAsked)
			{
				...
				//$indiceAsked =$indiceAsked-1;
				$line = $objectsrc->lines[$indiceAsked];
				...
				if ($line->fk_product > 0)  // If predefined product
				{
					...
					if (!empty($line->label)) {
						...
						print '<input type="hidden" name="comment'.$indiceAsked.'" value="'.$line->desc.'">';
					}
				...
				if (!empty($extrafields))
				{
					...
					$rline = new CommandeFournisseurDispatch($db);

					//$extrafields->fetch_name_optionals_label($e->table_element);
					//$line->id=$lines[$i]->id;
					//$line->fetch_optionals(); // fetch extrafields also available in orderline
					//$extrafields->fetch_name_optionals_label($srcLine->table_element);
					//$srcLine->id = "10069";

					//$srcLine->id = $lines[$i]->id;
					//$srcLine->fetch_optionals(); // fetch extrafields also available in orderline
					$rline->array_options = array_merge($rline->array_options, $line->array_options);
					$rline->array_options['options_vppo'] = "$objectsrc->id";
					//$line->array_options['options_cs'] = $objectsrc->socid;

					print $rline->showOptionals($extrafields, 'edit', array('style'=>'class="oddeven"', 'colspan'=>$colspan), $indiceAsked);

					//$srcline_id=$objectsrc->lines;
					//print $line->id;
				}
...
/* *************************************************************************** */
{
	...
	if ($object->id > 0)
	{
		...
		print '<a class="tabTitle"><span class="fas fa-dolly imgTabTitle flip" style=" color: #a69944;" title="Reception"></span> <span class="tabTitleText">Reception</span></a><div class="inline-block tabsElem tabsElemActive"><!-- id tab = reception --><a id="reception" class="tabactive tab inline-block" href="/custom/vptop/reception_card.php?id='.$object->id.'">Reception card</a>';
		print '</div><div class="inline-block tabsElem"><!-- id tab = contact --><a id="contact" class="tabunactive tab inline-block" href="/reception/contact.php?id=5">Contacts/Addresses</a>';
		print '</div><div class="inline-block tabsElem"><!-- id tab = note --><a id="note" class="tabunactive tab inline-block" href="/reception/note.php?id=5">Notes</a>';
		print '</div></div>';
		...
		// Reception card
		$linkback = '<a href="'.DOL_URL_ROOT.'/custom/vptop/reception_list.php?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
		...
		for ($i = 0; $i < $num_prod; $i++)
		{
			...
			if ($lines[$i]->fk_product > 0)
			{
				...
				if (!empty($lines[$i]->label)) {
					...
					print $text.' '.nl2br($lines[$i]->comment);
				}
	...
	if (($user->socid == 0) && ($action != 'presend'))
	{
		...
		if (empty($reshook))
		{
			...
			if ($object->statut == Reception::STATUS_VALIDATED && $user->rights->reception->creer) {
				print '<div class="inline-block divButAction"><a class="butAction" href="/custom/vptop/reception_card.php?id='.$object->id.'&amp;action=modif">'.$langs->trans('Modify').'</a></div>';
			}
